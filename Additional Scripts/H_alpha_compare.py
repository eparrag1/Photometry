...
def plotter(x,y,add,phot,crop,crop_no,label,color,z,rv = 3.1,av = 0.19):
    ...
    #logged_y = y
    """
    nonnan = (np.where(np.isnan(logged_y) == False))
    logged_y = logged_y[nonnan]
    x = x[nonnan]
    logged_y = logged_y - np.mean(logged_y)
    """
...
if __name__ == '__main__':
    ...
    x = x[a]
    y = y[a]
    T = plotter(x,y,1,phot0701,'',0,'2019hcc','black',0.044)
    excel('2018bsz',0.7,0.026665,1,'halpha','','',0)
    excel_II('SN2009dd',0.65,'',0.00246,0,0,'halpha')
    excel_II('SN1999ca',0.6,'',0.00931,5,0,'halpha')
    excel_II('SN2007ck_06_09',0.4,'',0.027,5,0,'halpha')
    excel_II('SN2008fq_11_20',0.2,'',0.0106,5,0,'halpha')
    excel('iPTF16bad_2',0,0.247,15,'halpha','','',0)
    #plt.legend()
    plt.show()
    """
    ...
    plt.figure('Display')
    x,y = spectra_plot('NTT_20190701',0,'none','no')
    T = plotter(x,y,1,phot0701,'post',600,'2019hcc','black',0.044)
    excel_II('SN1999ca',0.55,'green',0.00931,5,100,'')
    excel_II('SN2008fq_11_20',-0.15,'purple',0.0106,5,100,'')
    excel('iPTF16bad_2',-1.2,0.247,15,'','brown','pre',600)
    excel('iPTF13ehe_2',-1.85,0.33,25,'','red','pre',600)
    ...
    #excel_II('Early_2018bsz',-1,'',0.026665,5,100,'')
    ...
    """
    plt.figure('SLSN Comparison')
    excel('2010kd',9,0.1,1,'','','',0)
    excel('2018bsz',8.5,0.026665,1,'','','',0)
    x,y = spectra_plot('NTT_20190701',0,'none','no')
    T = plotter(x,y,-1.1,phot0701,'post',600,'2019hcc','black',0.044)
    plt.legend()
    plt.show()
    
    
    plt.figure('SLSN late H Comparison')
    
    excel('iPTF13ehe_2',11,0.33,15,'','')
    excel('iPTF15esb_2',0,0.224,15,'','')
    excel('iPTF16bad_2',14,0.247,15,'','')
    plt.legend()
    plt.show()
    ...
    plt.figure('Moderates')
    excel_II('SN2008fq_11_20',0,'black',0.0106,5,100,'')
    excel_II('SN1999ca',0.7,'black',0.00931,5,100,'')
    excel_II('SN2014G_02_06',-1.3,'black',0.004503,5,100,'')
    excel_II('SN2007ck_06_09',-0.8,'black',0.027,5,100,'')
    
    x,y = spectra_plot('NTT_20190701',1,'none','no')
    T = plotter(x,y,1,phot0701,'post',600,'2019hcc','black',0.044)
    plt.show()
    """
