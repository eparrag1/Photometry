...
#a = np.where((x<7200) & (x>6400))
#x = x[a]
#y = y[a]
plotter(x,y,1,phot0725,'',0,'2019/07/25','black',0.044)
plt.show()
#y = moving_average(3,y)
x,y = plotter(x,y,1,phot0725,'',0,'2019/07/25','black',0.044)

a = np.where((x<6850) & (x>6300))
x = x[a]
y = y[a]
...
def multi_lorentzian1(x,*pars):
    l1 = lorentzian(x, pars[0], pars[1], pars[2])
    cont = pars[3]
    return l1 + cont
...
#guess3 = [-0.1, 6400, 20, 0.1, 6500, 20,0.1, 6600, 10,0.05]
#guess3 = [0.1, 6500, 20, 0.1, 6550, 20,0.1, 6600, 10,0.05]
guess3 = [0.3, 6500, 20, 0.1, 6550, 10,0.05, 6600, 10,0.05]


popt, pcov = curve_fit(multi_gaussian3, x, y, guess3)

plt.figure()
plt.plot(x, y, '-', linewidth=2, label='Data')
gaussy = multi_gaussian3(x, *popt)
plt.plot(x, multi_gaussian3(x, *popt), 'r--', linewidth=2, label='Fit')
print(chisquare(gaussy, f_exp=y))


plt.plot(x,gaussian(x, popt[0], popt[1], popt[2]) + popt[9])

velocity(np.abs(popt[1]-6563.3),6563.3)
velocity(np.abs(popt[7]-6563.3),6563.3)
plt.plot(x,gaussian(x, popt[3], popt[4], popt[5])+ popt[9])
plt.plot(x,gaussian(x, popt[6], popt[7], popt[8])+ popt[9])
plt.axvline(x=6563.3, color = 'black', linewidth=1)
...
print('The mean value of blue-shifted emission of -2000km/s')
print('These both seem about right')
...
plt.plot(x, y, '-', linewidth=2, label='Data')
gaussy = multi_gaussian3(x, *popt)
plt.plot(x, gaussy, 'r--', linewidth=2, label='Fit')
print(chisquare(gaussy, f_exp=y))
print(popt[7])
velocity(np.abs(popt[7]-6563.3),6563.3)
...
guess2 = [0.2, 6510, 30,0.2, 6555, 5,0.05]

popt, pcov = curve_fit(multi_gaussian2, x, y, guess2)

plt.figure()
plt.plot(x, y, '-', linewidth=2, label='Data')
gaussy = multi_gaussian2(x, *popt)
plt.plot(x, gaussy, 'r--', linewidth=2, label='Fit')

print(chisquare(gaussy, f_exp=y))

plt.plot(x,gaussian(x, popt[0], popt[1], popt[2]) + popt[6])
plt.plot(x,gaussian(x, popt[3], popt[4], popt[5])+ popt[6])
plt.axvline(x=6563.3, color = 'black', linewidth=1)
...
popt, pcov = curve_fit(multi_gaussian1, x, y, [0.5,6560,30,0.05])

plt.figure()
plt.plot(x, y, '-', linewidth=2, label='Data')
gaussy = multi_gaussian1(x, *popt)
plt.plot(x, gaussy, 'r--', linewidth=2, label='Gaussian')
plt.axvline(x=6563.3, color = 'black', linewidth=1)
...
popt, pcov = curve_fit(multi_lorentzian1, x, y, [0.5,6560,30,0.05])

lorentz = multi_lorentzian1(x, *popt)
plt.plot(x, lorentz, 'b--', linewidth=2, label='Lorentzian')
plt.axvline(x=6563.3, color = 'black', linewidth=1)

print(chisquare(lorentz, f_exp=y))
