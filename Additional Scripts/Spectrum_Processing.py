...
def analysis(x,y,phot,mask1,mask2,line,custom,xmin,xmax,z = 0.044,rv = 3.1,av = 0.19):
    ...
    velocity,pew = vp.spectra_plot_xy(x,y,'yes','0725',mask1,mask2,line,custom,xmin,xmax)
    print(pew)


    return(velocity,pew)
    
def plotter(x,y,add,phot,crop,crop_no,label,color,linewidth,alpha,z = 0.044,rv = 3.1,av = 0.19):
#def plotter(x,y,add,phot,crop,crop_no,label,z,rv = 3.1,av = 0.19):
    ebv = av/rv
    new = cardelli(x,av,rv)
    x = x/(1+z)
    y = np.power(10,(0.4*new))*y
    ...
    if crop == 'post':
        ...
        x = x[:crop_no]
    #plt.figure('Cardelli Mangled')
    plt.plot(x,2.5*np.log10(y)+ add, label = label, linewidth = linewidth, color = color, alpha = alpha)
...
""" -----------THE FULL SPECTRUM PLOTS--------  """


x,y = spectra_plot('NTT_20190609',0,'none','no')
plotter(x,y,0,phot0609,'none',0,'06/09/2019','red',0.5,0.3)
plotter(x,moving_average(5,y),0,phot0609,'none',0,'06/09/2019','black',0.75,1)

data = np.genfromtxt('/Users/eleonoraparrag/Documents/Spectroscopy/2019hcc_20190621_SOAR.txt', dtype=None)
x =  data[:,0]
y =  data[:,1]
plotter(x,y,-0.7,phot0621,'pre',150,'06/21/2019','red',0.5,0.3)
plotter(x,moving_average(5,y),-0.7,phot0621,'pre',150,'06/21/2019','black',0.75,1)



x,y = spectra_plot('NTT_20190701',0,'none','no')
plotter(x,y,-1.5,phot0701,'post',600,'07/01/2019','red',0.5,0.3)
plotter(x,moving_average(2,y),-1.5,phot0701,'post',600,'07/01/2019','black',0.75,1)



x,y = spectra_plot('NTT_20190725_6',0,'none','no')
plotter(x,y,-0.7,phot0725,'none',0,'07/25/2019','red',0.5,0.3)
plotter(x,moving_average(5,y),-0.7,phot0725,'none',0,'07/25/2019','black',0.75,1)



x,y = spectra_plot('Extended_background/NTT_20190822',0,'none','no')
plotter(x,y,-2.9,phot0822,'none',0,'08/22/2019','red',0.5,0.3)
plotter(x,moving_average(5,y),-2.9,phot0822,'none',0,'08/22/2019','black',0.75,1)


x,y = spectra_plot('NTT_20191127_2',0,'none','no')
plotter(x,y,-7.5,[],'none',0,'11/27/2019','red',0.5,0.3)
plotter(x,moving_average(5,y),-7.5,[],'none',0,'11/27/2019','black',0.75,1)
